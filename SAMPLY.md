...
```bash
just profile                          # default: 20 iterations, 10k nodes, opens browser
just profile-save                     # save profile.json.gz instead of opening browser
just profile 5000 50                  # 5k-node problem, 50 iterations
just profile-save 5000 50             # same, but save only
just profile-extract                  # extract profile.json.gz → profile.json
just profile-extract my.json.gz out.json  # custom src/out paths
```

## Viewing results

- **Live**: By default, the profiler opens the Firefox Profiler at `http://localhost:3000+`.
- **Saved**: Use `just profile-save` to write `profile.json.gz`, then either run `just profile-extract` and upload the resulting `profile.json` to https://profiler.firefox.com/, or drag-and-drop the `.gz` file directly.
...
- **Empty symbols / `[unknown]` in profile**: The binary is built with `--profile profiling` (defined in `Cargo.toml`), which keeps debug symbols and disables stripping. If symbols are still missing on macOS, generate a dSYM bundle manually:
  ```bash
...
```bash
samply record ./cs2/cs2 < testdata/sample.inp
```

To save the profile for later viewing instead:

```bash
samply record --save-only -o cs2-profile.json -- ./cs2/cs2 < testdata/sample.inp
samply load cs2-profile.json
```

### 3. Tips

- **Use a large input** for meaningful samples. Small inputs finish in <1ms and produce very few samples. Generate larger GOTO problems with `just benchdata` or use a bigger DIMACS file.
